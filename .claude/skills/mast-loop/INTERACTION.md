...
- **`port-topology-demand.json`** (PRIMARY — value-ranked pick surface). Sections `witnessed_stems` / `declared_stems` / `holes` / `supergroups`, each entry `{ concept, kind, status, demand:{ witnessed?, corpus? }, priority?, matched_payoffs[], note? }`.
  - **`holes[]` and `declared_stems[]` are EMPTY as of 2026-07-20** (ADR-0004 §7 / issue #26). The topology no longer stores a hand-declared backlog or a declared-but-unwitnessed stem spine — every one of the six declared holes had been witnessed, and the declared spine was strictly subsumed, so both were deleted rather than left to drift. Issue #32 restores a backlog as a real set difference (proposed − witnessed) computed at report time. Until then, **pick from `witnessed_stems[]` and from the parse/projection gaps**, not from a stored hole list.
  - **Honest caveat, encoded in the report.** `demand.corpus` measures *payoff*-side demand, so **enabler** concepts (cost modification, restriction grants) read low/zero even when structurally important. Do not down-rank an enabler because its corpus demand is 0; that is the metric's known blind spot, not a verdict. (Coarse-token over-counts — e.g. `library` catching both search and selection — are the same coarseness; cross-check `matched_payoffs`.)
...
- **`port-topology.cited.json`** (STRUCTURE — what exists). **100% witness-derived since 2026-07-20** (ADR-0004 §7 / issue #26): `stems` (every entry `status: witnessed` + `witnesses[]`), `attribute_axes` (the stems carrying each axis + the value lattice actually seen), plus the un-witnessable scaffold vocabulary `supergroups` / `event_verbs` / `kinds`. The `aliases` and `holes` sections and the per-stem `unpredicted` flag NO LONGER EXIST — they were the hand-declared half, and a hand-typed `status: sought` inside a generated file was the bug ADR-0004 was written to end. Read this to see the shape you are adding to. The lean `port-topology.json` strips `witnesses`; use `.cited` when you need them.
...
**Pick heuristic.** Prefer a concept a single card or a known pairwise/combo can witness (cheapest to author) whose absence blocks real combo demand — read that demand off `combo-anchor-report.json` and `port-label-census.json` (`topProjectionGaps`), since the stored hole backlog is gone (#26) and its computed successor is #32. A gap that a popular, already-parsing card witnesses is the highest-leverage pick.
