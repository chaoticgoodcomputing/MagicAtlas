...
Step 1   Pick N families DEMAND-first: top parser-family hubs from combo-anchor-report.json by
         ...
         sharing one parser failure point). Skip empty-oracle-text (data-layer) hubs. Respect hot-file caps.
...
- **`combo-anchor-report.json` → `topAnchors[]`** (PRIMARY — demand-side) — the unparsed hub cards ranked by **`popularityMass`** (summed popularity of every combo the card gates). Each anchor carries: `card` + `typeLine`; `blockedComboCount` and **`soleBlockerCount`** (combos where this hub is the ONLY unparsed card — parse it alone and they become parse-ready, the highest-leverage signal); **`blockReason`** — **`parser-family`** (dispatch it), **`empty-oracle-text`** (the DFC/MDFC/Room ingest gap — a DATA-LAYER fix: flatten the faces into OracleText at ingest, NOT a worker family; do not dispatch), or **`missing-from-corpus`** (outside commander-legal scope — skip); `topPayoffs` (what its combos do); and `coStars[]` (the neighborhood, each `alsoUnparsed`-flagged: true = close together in the batch, false = lights up FREE once the hub lands). Value axis is the same `popularityMass` `fusedScore` folds in — but per **card** (demand) not per template (supply), so it surfaces the high-degree multi-clause legends (Mikaeus gates ~1,115 combos, sole-blocker in ~980) that `fusedScore` structurally under-ranks (one card spread across many templates → low `fractionalYield`). The report's summary also splits total blocking mass across the three reasons — the `empty-oracle-text` share (~8% / ~76 cards) quantifies the standing data-layer fix. **This is where the loop's objective is picked; L2% is the byproduct.**
