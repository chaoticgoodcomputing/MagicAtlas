...
Read `tests/magic-ast-tests/Data/_08_Reporting/triage-report.json`. Three surfaces inform family-picking — weigh all three:

- **`topYieldClusters[]`** (data-derived) — lexical-template clustering with greedy set-cover yield projection. Each carries `template`, `marginalYield`, `cumulativeYield`, `exemplars[]`. Finds the *big pre-named pattern missing entirely* gap (basic tapped lands, single-keyword reminder text) that never got hand-coded into `FallbackParser.InferFailurePattern`. Expand to ~20 entries for a full batch.
- **`topGaps[]`** (pattern-named) — each carries `pattern` AND `lastAttemptedRule`. A **family** is a `(pattern, lastAttemptedRule)` cluster. Sharp shape names you can brief in a paragraph. Bail signal: if `lastAttemptedRule` is `null` or `FallbackParser.*`, the cluster is too coarse — pick another.
- **`topGapsByLineFrequency[]`** (line-frequency ranked) — high-volume bail points with broad corpus impact even when they don't flip whole cards. Surfaces classifier-routing and prefix-stripping fixes that dominate by line frequency but never appear as top yield clusters by card count.

**Heuristic:** start with line-frequency for broad parser-surface opportunities, cross-reference yield clusters for card-level projection; `topGaps[]` bridges them. `marginalYield` numbers are advisory upper bounds — orchestrator judgment decides.

For each family: select **1-3 fixtures** (from `exemplars[]`, or `candidateLines[]` sorted by `cleanlinessScore` ascending; skip `alreadyHandParsed: true`). The low count is deliberate — N agents × 5 fixtures is unsustainable merge overhead, and coverage-per-fixture is the optimization target. **Diversity check:** the 1-3 should vary the dimensions the parser surface must handle, not be near-duplicates. When triage exemplars are stacked with multi-keyword legendaries, pre-curate cleaner single-line non-legendaries from `tests/magic-ast-tests/Data/_01_Raw/Datasets/External/oracle-cards.json` with a `jq` regex on the cluster template.
