...
> **This is the Parse track of the unified [`mast-loop`](../mast-loop/SKILL.md).** The loop has two tracks — Parse (this doc) and [Interaction](../mast-loop/INTERACTION.md) (ports, edges, taxonomy, and wrong-port correction). Enter through `mast-loop` and pick a track by objective; this doc is the authoritative Parse-track detail, and its "three kinds of effort" §below is where the engine currencies were promoted from. The shared machinery (worktree/serial-merge, judges, gates, fan-out, model policy) is described here and referenced by the umbrella.
...
1. **Parse-family** (the workhorse) — advance a card up the **fidelity ladder** (`GlobalMetrics.fidelity`): **L0** (an `IUnparsed` hole) → **L1** (a typed ability shell whose effect interior is a deferred `UnstructuredEffect` residual — accounted, not dropped) → **L2** (fully structured). The parsers already emit L1 shells for classified-but-unstructured clauses (confidence-gated), so most cards are ≥ L1 by construction; **the honest coverage metric is `l2Coverage` (fully structured), NOT `cardCoverage` (which counts L1+L2 and is inflated by shells)**. Two pick surfaces: **`topYieldClusters[]`** (ranked by `fusedScore`) closes the remaining **L0** cards (a gold fixture + parser rule per family); **`topResidualClusters[]`** is the **L1→L2 burn-down** — fragment families over the `UnstructuredEffect` residual interiors, keyed by normalized effect FRAGMENT (not whole line), so ONE new effect rule closes it across *every* shell that carries it (e.g. "create a X token" spans dozens of triggered/activated/spell shells). Both ranked by the same fused combo value. **Signal:** `l2Coverage` ↑ / residual mass ↓.
...
```
Step 0   Pre-flight: confirm execute mode + worktree base + refresh triage (nx run mast:interaction-triage; then mast:run; then mast:combo-anchors for the demand surface).
...
Step 1   Pick N families DEMAND-first: top parser-family hubs from combo-anchor-report.json by
         popularityMass, each resolved to its buildable topYieldClusters template (1-3 fixtures
         sharing one parser failure point). Skip missing-from-corpus hubs. Respect hot-file caps.
...
- **Refresh triage — ORDER MATTERS for the fused pick surface.** Run `nx run mast:interaction-triage` **before** `nx run mast:run`. The interaction slice regenerates `interaction-triage-report.json` (the `allComboBlockingCards` value map); `mast:run`'s `AggregateTriageReport` then reads that file and folds each card's combo-popularity mass into the cluster `fusedScore`. Run them the other way (or skip the interaction slice) and the aggregate falls back to the *previous* cycle's value map — still valid (combo popularity is stable batch-to-batch, and the fusion degrades to plain `fractionalYield` if the file is absent), just one refresh stale. The value map lags the corpus parse by at most one refresh either way, which is negligible for ranking. (Downstream, Step 8's `mast:interaction-triage` re-run keeps the map current for the *next* cycle's Step 0.) **Then `nx run mast:combo-anchors`** to refresh the Step-1 demand surface (`combo-anchor-report.json`) — it MUST run AFTER `mast:run` (it reads the fresh `parse-records.json`); the same `mast:run`-before-anything ordering applies. (The old forced-fresh `rm parse-records.json` step is obsolete — see the Step 7-8 note below.)

### Step 1 — Pick families

**Pick DEMAND-first.** The PRIMARY pick surface is `tests/magic-ast-tests/Data/_08_Reporting/combo-anchor-report.json` (`nx run mast:combo-anchors`) — the unparsed **hub cards** ranked by the combo value each gates. `triage-report.json`'s `topYieldClusters[]` is the SUPPLY-side complement: it turns a chosen hub into a *buildable family* (its failing template, clean exemplars, the homogeneity gate). The anchor says **which card is worth parsing** (the loop's real objective — reconstructed combos); the cluster says **how to parse it**. Read both. (The two `topGaps` lists remain diagnostics.)

- **`combo-anchor-report.json` → `topAnchors[]`** (PRIMARY — demand-side) — the unparsed hub cards ranked by **`popularityMass`** (summed popularity of every combo the card gates). Each anchor carries: `card` + `typeLine`; `blockedComboCount` and **`soleBlockerCount`** (combos where this hub is the ONLY unparsed card — parse it alone and they become parse-ready, the highest-leverage signal); **`blockReason`** — **`parser-family`** (dispatch it — this is ~all the value; double-faced cards are parser-families too, they parse from composed `CardFaces`), **`missing-from-corpus`** (outside commander-legal scope — skip), or **`empty-oracle-text`** (genuinely textless even after composing faces — ≈0 in scope; skip); `topPayoffs` (what its combos do); and `coStars[]` (the neighborhood, each `alsoUnparsed`-flagged: true = close together in the batch, false = lights up FREE once the hub lands). Value axis is the same `popularityMass` `fusedScore` folds in — but per **card** (demand) not per template (supply), so it surfaces the high-degree multi-clause legends (Mikaeus gates ~1,115 combos, sole-blocker in ~980; the DFC Eirdu gates ~1,695) that `fusedScore` structurally under-ranks (one card spread across many templates → low `fractionalYield`). **This is where the loop's objective is picked; L2% is the byproduct.**
- **`topYieldClusters[]`** (SUPPLY-side — the template resolver) — unparsed lines clustered by normalized lexical template, ranked by **`fusedScore`** — parse-proximity **weighted by the downstream combo value each surface unblocks**. Each cluster is a **buildable family** — one normalized template → one parser surface — and carries: `template`; **`fusedScore` (the primary ranking key = `fractionalYield × (1 + interactionValueScore)`)**; `fractionalYield` (proximity-weighted parse yield: each card contributes `1/(distinct templates on it)`, so it ranks high when it's the last-or-near-last missing piece across many nearly-complete cards); `directYield` (whole-card flips, a hard floor); **`comboBlockedCount` + `comboPopularityMass` (the combo-value axis — how many CSB combos, and how much popularity mass, this surface's cards gate, joined from the InteractionTriage `allComboBlockingCards` overlay with the same `1/N` attribution as `fractionalYield`)**; **`interactionValueScore` (`log10(1 + comboPopularityMass)` — the bounded value boost, 0 when the surface unblocks no known combo)**; `dominantPattern` + `dominantLastAttemptedRule` (the "where it fails" navigation hint — which parser the template bails in); `dominantShare` (diagnostic-spread homogeneity in [0,1] — see the gate below); and `exemplars[]` (each with `input` DTO and `alreadyHandParsed`, ready to hand-parse). ~50 entries deep, so the long tail (small/partial-card families like specific triggered abilities) is visible, not just the top whole-card flips. **The template is the family unit; `dominantPattern`/`dominantLastAttemptedRule` are annotations, not the key** — this is what splits coarse buckets (e.g. "UnparsedTriggered" — proliferate-trigger, roll-a-d20, play-a-card-trigger each surface as their own cluster). **Fusion is graceful:** with no InteractionTriage value map present, every `interactionValueScore` is 0, `fusedScore` collapses to `fractionalYield`, and the surface ranks exactly as the pre-fusion loop did — so the pick surface is never *worse* than parse-yield-only, only better-informed when the overlay is fresh.
...
**Heuristic — demand-first, template-resolved.** Take the top **`parser-family`** anchors from `combo-anchor-report.json` by `popularityMass` (skip `missing-from-corpus` → out of scope, and the ≈0 `empty-oracle-text` → genuinely textless). Double-faced hubs (Eirdu, Walk-In Closet) ARE parser-families — they parse from composed `CardFaces`; hand the worker the `CardFaces` block (Step 2). For each picked hub, resolve its *buildable family*: find the `topYieldClusters[]` cluster its failing line belongs to (for a multi-clause legend spanning several templates, the cluster of its highest-value unparsed line) to get the exemplars, `dominantPattern`/`dominantLastAttemptedRule`, and the homogeneity gate below. The anchor chooses the card by combo value; the cluster supplies the parser surface + the clean-exemplar discipline (unchanged below). Prefer hubs with high `soleBlockerCount` (parsing one card flips that many combos to parse-ready) and whose `coStars` are mostly already-parsing (they light up free); when several picked hubs share `alsoUnparsed` co-stars, batch them together — the neighborhood closes as a unit. **Fall back to pure `fusedScore` top-down** only when the anchor surface is exhausted at the current tier (no combo-unblocking hub left) — then the `topYieldClusters[]` sub-signals rank the residual parse-hygiene work (high `directYield` flips whole cards; high `fractionalYield` chips many toward done; near-zero `comboPopularityMass` = pure hygiene, correctly below any combo-unblocking surface). Multi-clause legends are heavier, less parallel, and often Opus — size the batch accordingly. Cross-reference the diagnostic gap lists only to understand *where* in the parser the work lands.

For each family: select **1-3 fixtures** from the cluster's `exemplars[]` (already ranked cleanest-first — genuinely-clean cards before `lossyRisk: true` ones, then by fewest other unparsed templates; skip `alreadyHandParsed: true`). **Heed `lossyRisk`.** An exemplar with `lossyRisk: true` parses one or more of its NON-target lines *lossily* — dropping a trigger/granted-ability with no diagnostic, so `otherUnparsedClusters` understates its risk (the `LossyParseAnalyzer` trigger-deficit signal; `GlobalMetrics.suspectedLossyCleanCards` counts the corpus-wide blind spot). Prefer a non-lossy exemplar; if you must use a `lossyRisk` card, do NOT tell the worker "the other lines already parse" — have it author the WHOLE-card gold and verify each line's parse, bailing if a non-target line is lossy (the Pest Rescuer / Keranos lesson). The low count is deliberate — N agents × 5 fixtures is unsustainable merge overhead, and coverage-per-fixture is the optimization target. (`OtherUnparsedClusters: 0` is low-risk, not zero-risk — a multi-clause non-target line can still fail and force a worker STOP; improving that signal is a triage concern, not a per-batch worry.) **Diversity check:** the 1-3 should vary the dimensions the parser surface must handle, not be near-duplicates. When exemplars are stacked with multi-keyword legendaries, pre-curate cleaner single-line non-legendaries from `tests/magic-ast-tests/Data/_01_Raw/Datasets/External/oracle-cards.json` with a `jq` regex on the cluster template.
...
Per-merge-group (not once at batch end) is the point: the lint reads the **merged source** directly, so it surfaces a concurrent duplicate at the **first** merge that introduces it, not after the whole batch has landed (closing the concurrent-duplicate hole — initiative 02 #2). A hard fail (duplicate within a family) is an unconditional HALT — rename. That half rides the core ring as `DiscriminatorUniquenessTests` (`nx run mast:test`), so the script is a convenience for worktrees without `dotnet` — not the enforcement point. Uniqueness is **per-base, not global** — cross-base reuse (`untap` as Effect+Cost+ReplacementEvent) is legitimate.

The **near-duplicate** half is no longer a gate (ADR-0004 issue #38). `discriminator-justifications.json` and `DiscriminatorNearDuplicateTests` are deleted: the rulings now sit on the discriminator attribute at its declaration site — `[OracleEffect("gift", NearDuplicateOf = new[] { "graft" }, Reason = "…CR citation…")]` — where a ruling cannot outlive the type it explains. The lint PRINTS every near-duplicate pair, marked explained or UNEXPLAINED, and exits 0; the same split is reported by `nx run mast:discriminator-governance`. If you add a discriminator that lands near an existing one, either rename it or record the ruling on the more specific type.
...
nx run mast:combo-anchors  # refresh the Step-1 demand surface (combo-anchor-report.json) — AFTER mast:run (reads fresh parse-records)
nx run bench:recall        # GATE: per-combo expected-tier whitelist (initiative 04, de-ratcheted) — HALT if any combo's tier drifts from combo-axis-expectations.json
nx run mast:recall-report                 # regenerate the wide product measurement (extended-recall-report.json)
bash tools/recall-log.sh {batch}          # append green/amber/missed + recall deltas to tracked docs/judgments/recall-log.jsonl; quote in the batch report
git add docs/judgments/recall-log.jsonl   # commit the product-recall snapshot with the batch
...
**No forced re-parse needed (ADR 0004 #22).** `nx run mast:run` re-parses the corpus automatically whenever the parser changed. The step cache key now folds in the first-party code each step actually executes — a `libs/magic-ast/` edit changes `ParseCorpus`'s key and everything downstream cascades. Measured: ~46 s when the parser changed, ~0.9 s when it did not. The old recipe (`rm Data/_07_ModelOutput/Datasets/parse-records.json` + `--only ParseCorpus`) is **retired**; the mechanism lives in `tests/magic-ast-tests/Infrastructure/StepCodeIdentity.cs` and is gated by `Tests/Pipeline/StepCacheKeyingTests.cs`.

**Escape hatch.** `--no-cache` still exists for the one case the key genuinely cannot see: a data file edited in place out of band. Flowthru fingerprints file items on `mtime:size`, not content, so a same-size edit that preserves mtime is invisible. Code changes are not that case — do not reach for `--no-cache` reflexively.

**GATE — combo expected-tier (initiative 04, de-ratcheted), Step 8.** `nx run bench:recall` runs the interaction engine over the eligible Commander Spellbook combos; **`ComboAxisExpectationTest` fails if ANY eligible combo's reconstruction tier drifts from its explicit pin in `tools/bench/MagicAtlas.Bench/combo-axis-expectations.json`** — a loud, per-combo gate (the end-product measure; parser-green ≠ product-green). A REGRESSION (e.g. Green→Amber) is an unconditional HALT: investigate before looping. An IMPROVEMENT (e.g. Missed→Amber) also fails — **update that combo's pin** to lock the gain (a named edit, never a silent baseline rewrite). `bench-report.json` is now a derived report, not the gate. **Put the tier summary (`Green / Amber / missed`) in the batch report**; the missed combos are the aligned worklist for the next pick (Step 1). **The gold-33 is the GATE, not the scoreboard** — its denominator is too small to show a batch's progress (a batch can unblock dozens of combos and move it by zero). For the batch SCOREBOARD, regenerate and quote the **wide measurement tier**: `nx run mast:recall-report` (= `dotnet run -- --flow CardAtlas`) emits `Data/_08_Reporting/extended-recall-report.json` (schema `ExtendedRecallReport`) — reconstruction recall over EVERY projection-ready combo (thousands), with `green / amber / missed`, `recallAtGreen`, `recallAtAmber`, and a **popularity-weighted recall** (are we reconstructing the *popular* combos?). It has no pins and is not a gate (it needs the corpus, so main-only); it is the number that actually moves per batch. **Then `bash tools/recall-log.sh {batch}`** appends the snapshot to the git-tracked `docs/judgments/recall-log.jsonl` and prints the delta vs the prior batch — the source of the batch report's "Product delta" block ([template in PIPELINE.md](PIPELINE.md#batch-report)); commit the log line with the batch. Report BOTH: the gold-33 tier line (regression gate) and the wide recall deltas (progress) — the latter is the objective, L2% is the proxy. (Free-text cleanliness — `GoldFreeTextWhitelistTests`, the de-ratcheted replacement for the old `DestringSinkRatchetTests` *count* baseline — rides the core ring in `nx run mast:test`: no gold may carry a free-text residual sink unless its exact `(card, sink)` pair is named on `tests/magic-ast-tests/Fixtures/whitelist-freetext.json`. Gates at Steps 5-6 automatically; no separate Step-8 call.)
...
- `ComboAxisExpectationTest` fails (an eligible combo's tier drifted from its pin in `combo-axis-expectations.json`) — a REGRESSION means a batch silently lost interaction reconstruction. HALT; find which merged branch's node-shape/projection change dropped the cycle (the sentinel snapshot diff from initiative 03 usually localizes it); re-pin only genuine improvements, never a regression (that would paper over a real loss).
...
| Combo-recall bench + per-combo expected-tier gate (end-product, Step 8) | `nx run bench:recall` (`tools/bench/MagicAtlas.Bench/`; gate = `combo-axis-expectations.json`; `bench-report.json` = derived report) |
