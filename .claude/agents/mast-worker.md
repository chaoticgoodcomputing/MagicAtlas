...
- Exception: if you edited a **shared file** (`AST/References/ObjectFilter.cs`, `Parsing/AbilityClassifier.cs`, `AST/Triggers/TriggerCondition.cs`, any of the four `Parsing/Parsers/*/[*]RuleHelpers.cs`, `Parsing/ConditionParser.cs`, or any parser body), run the full project once (`dotnet test tests/magic-ast-tests/MagicAtlas.Ast.Tests.csproj --nologo`) to check you didn't regress siblings, then report that you did so.
- **ANCHOR your matchers; sweep siblings (the #1 FAIL class).** A regex that matches its surface phrase as a **substring** inside a more-specific sibling trigger silently drops that sibling's filter — the overfit that FAILed Mindcrank, Rings, Hapatra. The full suite stays GREEN because the colliding siblings have no gold fixtures, so this is invisible to your targeted test. Defenses, both required: (1) anchor every matcher (`^…$` or tightly bounded) by default; (2) if you add/edit a non-anchored matcher OR touch a `*RuleHelpers.cs`, grep the corpus oracle text for other cards sharing that surface phrase and confirm none is newly mislabeled or has filters dropped — report that you swept. Prefer a NEW reflection-discovered rule file over editing a shared `*RuleHelpers` (it cannot collide).

## Gold-AST authoring rules (the fixture is the committed FAILING test; extend the parser to MEET it, never edit it to pass)
- **`Input` is PRE-SEEDED by the orchestrator from the corpus — copy `Input.OracleText` BYTE-FOR-BYTE.** Do NOT normalize typography: keep curly quotes (U+201C/U+201D), apostrophes (U+2019), em/en-dashes EXACTLY as given; never fold to ASCII. A single substituted character is **silent in your worktree** (the oracle-fidelity test skips — the corpus is absent here) and only fails the orchestrator's post-merge gate. "Copy verbatim" is violated by a one-character "cleanup". Author only the `Output` AST + the parser.
...
- **New discriminator → projection decision (initiative 03).** If your work adds a new discriminator — a new `[OracleEffect]`/`[OracleCost]`/… node, or a new `TriggerEvent`/restriction enum value — you MUST make a PortWalk projection decision, or `PortWalkExhaustivenessTests` fails the suite. Either (a) add a semantic projection: a case in `libs/mast-interaction/PortGraph.cs` dispatch **and** the matching entry in `PortWalkProjection`; or (b) if it is genuinely inert for interactions, add a one-line justified entry under the right dimension in `libs/mast-interaction/known-coarse-projections.json`. Prefer (a) when a flow rule could read it; (b) is the conscious "no recall here, and that's correct" choice. Report which you chose. **If you are wiring a projection into a flow edge (closing a missed combo family), follow the reference pattern in [`libs/mast-interaction/docs/adding-a-flow-arm.md`](../../libs/mast-interaction/docs/adding-a-flow-arm.md) exactly — it is the judge-verified exemplar (the life arm) and lists the anti-patterns (don't encode the cross-card connection in the AST; don't fudge a GREEN; carry a non-null Subject so a scoped resource never hits the scalar null-default GREEN; shrink the coarse allowlist).**
