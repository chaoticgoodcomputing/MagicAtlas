...
- **Trigger ≠ effect.** A triggered ability is a `Trigger` node (timing + event + filter) PLUS an `Effects[]` list — never bake the trigger/timing into the effect. "When/Whenever [X], [do Y]" → `Trigger{...}` + effect `Y`. ("As [this] enters …" is a *static* replacement ability per CR 603.6d/614.1c — `Kind: static`, not triggered — but even then the effect node names the *action*, not the timing.)
...
- Fixture schema = `{"Input": <CardInputDTO>, "Output": <gold CardOutputAST>}`. Study a similar existing fixture (e.g. `tests/magic-ast-tests/Data/HandParsedCards/AggressiveMammoth.json` for anthems/`gainAbility`/filters). `Abilities[]` use `Kind`; effects use `EffectType`; targets use `Kind`+`Filter`.
...
## Family contract
Close the family via ONE consolidated parser surface. A trigger family may legitimately be ONE `[TriggerConditionRule]` + ONE paired effect rule; a keyword family is its `[Keyword]` + effect node. But if you find yourself writing several *different* `TryParse*` methods for genuinely different shapes, STOP and report a sub-pattern breakdown (bailing refines triage — it is not failure).
