...
- **The cycle set is the canonical derived index (banked design).** [`two-layer-cycle-engine.md`](../../libs/mast-interaction/docs/two-layer-cycle-engine.md) proposes enumerating cycles over the *distinct-label* graph (the bounded atom set — measured at **545 cycle-relevant labels over 29,615 cards**, 54× dedup), then instantiating + tiering per candidate. That label-cycle set is exactly the derived artifact this ADR versions and incrementally recomputes: invalidation keyed on `(label, version)` is *tighter* than the per-card neighborhood §2 bounds, and "all cycles a card is in" becomes `card → labels → label-cycles → instantiate`. The two are complementary — that doc owns the engine half; this ADR owns the versioning/invalidation/atomic-swap half.
