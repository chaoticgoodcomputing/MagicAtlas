...
public sealed class ClauseSplitter
{
  ...
  public IReadOnlyList<OracleClause> Split(string oracleText)
  {
    ...
    for (var i = 0; i < paragraphs.Count; i++)
    {
      ...
      // Borderpost cycle's alternative-cost line ("You may pay {1} and return a basic
      // land you control to its owner's hand rather than pay this spell's mana cost.")
      // is a card-casting annotation (CR 118.9 alternative cost; CR 604.5 — functions
      // while the spell is on the stack), not an oracle ability. AttributeExtractor
      // parses the cost and emits AlternativeCostsAttribute instead. Skipping here
      // prevents a spurious UnparsedAbility/AmbiguousStructure failure in
      // Oracle.Abilities for the cost line.
      if (IsBorderpostAlternativeCostPrefix(paragraphText))
      {
        continue;
      }
      ...
      // "Sacrifice a creature" alternative-cost family's line ("You may sacrifice a
      // nontoken red creature rather than pay this spell's mana cost.") is a
      // card-casting annotation (CR 118.9 alternative cost; CR 604.5 — functions while
      // the spell is on the stack), not an oracle ability. AttributeExtractor parses
      // the cost and emits AlternativeCostsAttribute instead. Skipping here prevents a
      // spurious UnparsedAbility/UnparsedSpell failure in Oracle.Abilities for the
      // cost line.
      if (IsSacrificeAlternativeCostPrefix(paragraphText))
      {
        continue;
      }
  ...
  /// <summary>
  /// Recognises the Borderpost cycle's "You may pay {1} and return a basic land you
  /// control to its owner's hand rather than pay this spell's mana cost." line. CR
  /// 118.9 (alternative cost) / CR 604.5 (functions while the spell is on the stack).
  /// AttributeExtractor parses the cost and emits AlternativeCostsAttribute instead.
  /// Kept narrow to this specific sentence so it does not swallow other
  /// "rather than pay this spell's mana cost" alternative costs handled elsewhere.
  /// </summary>
  private static bool IsBorderpostAlternativeCostPrefix(string text) =>
  ...
  /// <summary>
  /// Recognises the "sacrifice a creature" alternative-cost family's "You may sacrifice
  /// a nontoken [color] creature rather than pay this spell's mana cost." line (e.g.
  /// Flare of Duplication). CR 118.9 (alternative cost) / CR 604.5 (functions while the
  /// spell is on the stack). AttributeExtractor parses the cost and emits
  /// AlternativeCostsAttribute instead. Kept narrow to this specific sentence so it does
  /// not swallow other "rather than pay this spell's mana cost" alternative costs
  /// handled elsewhere (Bestow, Borderpost, Pitch).
  /// </summary>
  private static bool IsSacrificeAlternativeCostPrefix(string text) =>
