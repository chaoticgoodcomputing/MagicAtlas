...
/// <summary>
/// "Target player shuffles their graveyard into their library." — targeted
/// whole-zone recycle (Reminisce).
///
/// <para>
...
/// <see cref="ObjectReferenceKind.Target"/> with a player filter — matching
/// the "target player" subject in oracle text. Sibling of
/// <see cref="EachPlayerShufflesGraveyardIntoLibraryRule"/> (which handles the
/// "each player" subject instead).
/// </para>
/// </summary>
[SpellRule]
public sealed class TargetPlayerShufflesGraveyardIntoLibraryRule : ISpellRule
{
  private static readonly Regex Pattern = new(
    @"^Target\s+player\s+shuffles\s+their\s+graveyard\s+into\s+their\s+library$",
