...
/// <summary>
/// "Regenerate target [filter]." — single-target regenerate spell (CR 701.19,
/// "regenerate [permanent]" keyword action). Mirrors <see cref="DestroyTargetSimpleRule"/>:
...
/// </summary>
[SpellRule]
public sealed class RegenerateTargetSimpleRule : ISpellRule
{
  // Simple bare card-type targets (fast path — preserves existing behaviour).
  private static readonly Regex SimplePattern = new(
    @"^Regenerate\s+target\s+(?<type>creature|artifact|enchantment|land|planeswalker|permanent)$",
    RegexOptions.Compiled | RegexOptions.IgnoreCase
  );

  // Richer filter: one or more words after "Regenerate target"
  // (covers color+type, subtype-only, non-prefix patterns).
  private static readonly Regex FilterPattern = new(
    @"^Regenerate\s+target\s+(?<filter>.+)$",
    RegexOptions.Compiled | RegexOptions.IgnoreCase
  );
  ...
  public bool TryMatch(string text, out Effect? effect)
  {
    ...
    // Fast path: bare card-type target (Regenerate target creature, etc.)
    var simple = SimplePattern.Match(text);
    if (simple.Success)
    {
      effect = new RegenerateEffect
      {
