...
internal static class TriggeredRuleHelpers
{
  ...
  public static ObjectFilter? ParseObjectFilter(string text)
  {
    ...
    // Rule 303.4c / 702.5: an Aura's "enchanted [type]" refers to the permanent it's
    // attached to. The attachment is recorded on the structured IsEnchanted axis (mirrors
    // IsSelf/IsToken); the card type remains "creature" per the oracle text. This is
    // descriptive — MAST records the oracle word, not a resolved attachment reference.
