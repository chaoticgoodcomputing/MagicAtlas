...
/// <summary>
/// "This [type] gains [basic landwalk] until end of turn." — a self-targeting
/// activated grant of a basic-land landwalk keyword for the rest of the turn
/// (Enslaved Scout "{2}: This creature gains mountainwalk until end of turn.").
///
...
/// self-grant branch of <see cref="GainAbilityEffectRule"/> (Priority 995) returns
/// null for them and the effect lands unstructured (L1). This dedicated,
/// fully-anchored rule structures the interior into the same
/// <see cref="GainAbilityEffect"/> shape those siblings emit, granting the
/// landwalk keyword's canonical structured expansion — identical to the printed
/// <c>MountainwalkKeyword</c> / <c>ForestwalkKeyword</c> combinator output
/// (an <see cref="EvasionEffect"/> whose <see cref="EvasionEffect.UnblockableCondition"/>
/// is <see cref="EvasionConditionType.DefendingPlayerControls"/> over the matching
/// basic land subtype).
///
...
/// so it claims only this exact self-grant phrase and never shadows another activated
/// effect. Sits above the generic <see cref="GainAbilityEffectRule"/> (Priority 995);
/// that rule cannot claim these keywords anyway (helper miss), so this is
/// collision-free.
/// </summary>
[ActivatedEffectRule(Priority = 997)]
public sealed class ThisCreatureGainsLandwalkEffectRule : IActivatedEffectRule
{
  ...
  // "This <type> gains <basic landwalk> until end of turn" — mirrors the self-grant
  // shape of GainAbilityEffectRule ("This\s+\w+\s+gains?"), pinned to the landwalk words.
  private static readonly Regex SelfGrantsLandwalkPattern = new(
    @"^This\s+\w+\s+gains?\s+(?<walk>forestwalk|islandwalk|swampwalk|mountainwalk|plainswalk)\s+until\s+end\s+of\s+turn$",
  ...
  public Effect? TryMatch(string effectText)
  {
    ...
    var match = SelfGrantsLandwalkPattern.Match(trimmed);
    if (!match.Success)
    {
      return null;
    }
