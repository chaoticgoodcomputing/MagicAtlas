...
/// <summary>
/// "Cumulative upkeep—Discard a card." (Vexing Sphinx) — the discard-cost-templated
/// printing of cumulative upkeep, decomposed into its full triggered ability per
/// CR 702.24a exactly like the draw-cost sibling
/// <see cref="CumulativeUpkeepDrawCardStaticRule"/> and the life-cost sibling
/// <see cref="CumulativeUpkeepPayLifeStaticRule"/>, but with the per-age-counter
/// cost being a card discard (<see cref="DiscardCost"/>) rather than
/// <see cref="DrawCardsCost"/> or <see cref="PayLifeCost"/>.
///
/// <para>
...
///
/// <para>
/// A dedicated sibling <c>[StaticRule]</c> — mirroring the split already
/// documented between <see cref="CumulativeUpkeepPayLifeStaticRule"/> and
/// <see cref="CumulativeUpkeepDrawCardStaticRule"/> (also the precedent of
/// <c>DashStaticRule</c>/<c>DashKeyword</c>) — rather than widening either
/// existing rule's regex: the em-dash "—Discard N card(s)." cost shape is a
/// distinct printed template from the mana-symbol shape ("Cumulative upkeep
/// {G}"), the life-cost shape ("—Pay N life."), and the draw-cost shape
/// ("—Draw N card(s)."), so this rule adds coverage for the discard-cost
/// template while leaving the existing rules (and their gold fixtures) completely
/// untouched. ANCHORED (^…$) on the full oracle line (cost text plus optional
/// trailing reminder) so it cannot match as a substring of a broader clause.
/// </para>
/// </summary>
[StaticRule(Priority = 60)]
public sealed class CumulativeUpkeepDiscardCardStaticRule : IStaticRule
{
  ...
  // Matches: "Cumulative upkeep—Discard a card." / "Cumulative upkeep—Discard N cards."
  // with optional trailing reminder text.
  private static readonly Regex _pattern = new(
    @"^\s*Cumulative\s+upkeep\s*—\s*Discard\s+(?<amount>a|an|\d+|one|two|three|four|five|six|seven|eight|nine|ten)\s+cards?\.?\s*(?<reminder>\(.*\))?\s*$",
  ...
  public IReadOnlyList<Ability>? TryParse(OracleClause clause, ClauseClassification classification)
  {
    ...
    if (!StaticRuleHelpers.TryParseSmallCount(match.Groups["amount"].Value, out var discardAmount))
    {
      return null;
    }
        ...
        [
          ...
          new PreventableEffect
          {
            ...
            Unless = new UnlessClause
            {
              ...
              Cost = new ScaledCost
              {
                PerUnit = new DiscardCost
                {
                  Filter = new ObjectFilter { CardTypes = ["card"] },
                  Quantity = LiteralQuantity.Of(discardAmount),
                },
