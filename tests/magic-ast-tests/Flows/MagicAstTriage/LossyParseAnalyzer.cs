...
/// e.g. Keranos's line 3 collapses two "Whenever …" triggers into a bare
/// <c>dealDamage</c> spell (deficit 2). Pest Rescuer's dropped token-granted ability
/// is deficit 1 before the fix, 0 after — so the signal clears once the loss is
/// closed. It is intentionally CONSERVATIVE (a deficit means a real drop; it does
/// not claim to catch every lossy shape — a dropped static with no trigger, like
/// Keranos's "Reveal …" line, is not counted, though such cards are usually flagged
/// anyway by an accompanying trigger drop).
/// </remarks>
