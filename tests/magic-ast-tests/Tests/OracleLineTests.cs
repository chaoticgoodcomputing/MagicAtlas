...
public class OracleLineTests
{
  ...
  public void ParseOracleLine_ShouldMatchExpectedAbility(OracleLineTestCase testCase)
  {
    ...
    // Track overall test result for ratcheting
    ...
    try
    {
      Assert.Multiple(() =>
      {
        ...
        var errors = result.Diagnostics.Where(
          d => d.Severity == MagicAST.Diagnostics.DiagnosticSeverity.Error
        );
        Assert.That(
          errors,
          Is.Empty,
    ...
    var testName = $"OracleLines/{testCase.SourceCard}/Line{testCase.LineNumber}";
    RatchetTestTracker.Instance.RecordResult(testName, testPassed);

    // Rethrow if test failed
    if (!testPassed && testException != null)
    {
      throw testException;
    }
  }
