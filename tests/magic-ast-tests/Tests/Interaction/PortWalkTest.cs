...
public class PortWalkTest
{
  ...
  // A {T} (tap) cost is a rate limit (CR 107.5 — once per untap), so it TAP-gates the ability's ports
  // (ADR-0002 §8) — the dischargeable gate, distinct from the hard Gated of an intervening-if/once-each-
  // turn. (The discharge — an untapper in the loop — lives in the engine, PortCycle.TapRenewed.)
  [Test]
  public void Tap_cost_tap_gates_its_ports()
  {
    var abilities = JsonNode.Parse(
      """
      [{"Kind":"activated",
