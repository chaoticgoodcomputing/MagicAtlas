...
/// <summary>
/// The <b>combo-anchored</b> pick surface for the mast-tdd-loop — the demand-side complement to
/// <see cref="InteractionTriageReport"/>'s <c>allComboBlockingCards</c>. Where the yield-cluster
/// surface (triage-report.json) ranks parse families by SUPPLY (how much L2 coverage a template
/// buys), this ranks the unparsed <b>hub cards</b> by the combo-popularity value each one gates —
/// so the loop can pick a card because reconstructing 1,000 real combos waits on it, not because it
/// shares a lexical template with many others. A high-degree multi-clause legend (Mikaeus) sinks on
/// the supply surface but tops this one.
///
/// <para>
...
///
/// <para>
/// Same name-join and <c>fullyParsed</c> definition as <c>ClassifyCombosStep</c>, so a hub here is
/// exactly a blocking card there. Measurement/pick surface only — never a gate.
/// </para>
/// </summary>
[FlowthruSchema]
